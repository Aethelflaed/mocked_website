...
CustomTeardown = MockedWebsite.create("http://example.org") do
  def self.teardown
    :foo
  end
end
...
class MockedWebsiteTest < Minitest::Test
  ...
  def test_respond_to_setup_and_teardown
    ...
    assert CustomTeardown.respond_to?(:setup)
    assert CustomTeardown.respond_to?(:teardown)
    assert_equal :foo, CustomTeardown.teardown
  end
